var messageMarketingItem = {
    ...
    requestPrepare: function(method, path, data) {
        ...
        var succ = function(prepare_list) {
                ...
                $(".effectNum").text(prepare_list.effectCount + "名会员");
                var storeNumLength = prepare_list.storeNameCountMap;
                // 筛选人数等于0时，不能进行分组
                if (prepare_list.effectCount == 0) {
                    ...
                    $('.quickResult .screenAgain').addClass("hideBox"); //隐藏错误信息
                    $('.quickResult #allNum').removeClass("redText"); //将全部数据的字标红
                    $('.quickResult #effectNum').removeClass("redText"); //将有效数据的字标红
                    ...
                    for (var i in storeNumLength) {
                        $(".storeNumList").append(
                            '<li> <span>' + i + '：' + prepare_list.storeNameCountMap[i] +
                            '</span></li>'
                        )
                    }
                }
        ...
        $(document).ajaxComplete(function() {
            ...
            $("#distribute" + getFlag).removeClass('hideBox');
        })
                ...
                if (crmGroupFirstClassList[i].type == 2) {
                    if (crmGroupFirstClassList[i].crmGroupList.length == 0) { //如果没有我的分组，则隐藏我的分组
                        ...
                        for (var j in crmGroupFirstClassList[i].crmGroupList) {
                            //从会员管理的电话营销进入到电话营销界面直接选中对应的分组
                            if (messageMarketingItem.groupId && messageMarketingItem.groupId == crmGroupFirstClassList[i].crmGroupList[j].id) {
                                $("#myGroup").append(
                                    '<label class="btn btn-primary active myGroupList">' +
                                ...
                                $("#myGroup").append(
                                    '<label class="btn btn-primary myGroupList">' +
                                    '<input type="radio" autocomplete="off" data-id="' + crmGroupFirstClassList[i].crmGroupList[j].id + '">' + crmGroupFirstClassList[i].crmGroupList[j].name + '(' + crmGroupFirstClassList[i].crmGroupList[j].num + ')' + '</label>'
                                );
                            }
                        }
    ...
    //快速筛选创建电话营销
    createCallMarketByGroup: function() {
        var data = {
            "mStId": messageMarketingItem.mainStoreId,
            "euId": messageMarketingItem.euId,
            "marketingName": $("#quickSelectTaskName").val(),
            "groupId": messageMarketingItem.groupId
        }
        var success = function() {
            location.href = "PhoneMarketingTaskList.html";
        }
        post("post", "/clCrm/api/crm/marketing/call", data, success);
    },

    //会员筛选创建电话营销
    createCallMarketBySearch: function() {
        var data = {
            "mStId": messageMarketingItem.mainStoreId,
            "euId": messageMarketingItem.euId,
            "marketingName": $("#searchTaskName").val()
        }
        var success = function() {
            location.href = "PhoneMarketingTaskList.html";
        }
        post("post", "/clCrm/api/crm/marketing/call", $.extend(data, messageMarketingItem.memberFilterData), success);
    },
    ...
    mouseEvent: function() {
        ...
        $("#screen0").click(function() {
            ...
            messageMarketingItem.requestPrepare('post', '/clCrm/api/crm/marketing/call/preparation', data);
        });

        //快速筛选：点击立即分配
        $(".quickBtns #taskConfirm").click(function() {
            messageMarketingItem.createCallMarketByGroup();
        });

        //会员筛选:点击立即分配
        $(".memberBtns #taskConfirm").click(function() {
            messageMarketingItem.createCallMarketBySearch();
        });
        ...
        $(".memberScreen .btn").each(function(index) {
            ...
            $(".delete").eq(index).on('click', function() {
                ...
                if (getItem == 1) {
                    ...
                    $("#distribute" + getFlag).addClass('hideBox');
                }
            })
        });
        // 点击自定义按钮显示相应的自定义框
        ...
        $("#screen1").on('click', function() {
            ...
            console.log(passPara());

            messageMarketingItem.requestPrepare('post', '/clCrm/api/crm/marketing/call/preparation', $.extend(data, messageMarketingItem.memberFilterData));
    ...
    quickChooseMemberAjax: function() {
        ...
        $('#quickCreatePhoneMarketingTable').DataTable({
            //以下是对表格获得数据的设置
            "dom": "Btlp",
            "destroy": true,
...
$(function() {
    ...
    messageMarketingItem.euId = $.cookie("employeeId");
    if (getUrlParam("groupId")) {
        messageMarketingItem.groupId = getUrlParam("groupId");
    }
